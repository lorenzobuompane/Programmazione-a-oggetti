...
//    	this.t.get(table).setnOccTable(this.c.get(partyId).getCount());
    ...
    public String estimateAvailable(int table) {
    	if (this.t.get(table).isOccupation()) {
    		if (this.t.get(table).hasBooked()) {
    			int i = this.c.get(this.t.get(table).getLastBooked()).getCount();
        		i = 40 + (5 * i);
        		int ore = 0;
        		int h=0;
        		for (h=i; h>=60; h=h-60) {
        		...
        		String time = this.t.get(table).getLastBookedTime();
        		String split[] = time.split(":");
    ...
    public List<TableOption> estimateWaiting(int partySize){
    	...
    	List<TableOption> list = this.t.values().stream()
    			.filter(x->{
    				if (!x.isOccupation() && x.gettableSize()>=partySize) {
    					x.setEstNumPeople(partySize);
    					return true;
    				}
    				else {
    					if (x.getTime().compareTo(this.closingTime)<=0 && x.gettableSize()>=partySize) {
    						x.setEstNumPeople(partySize);
    						return true;
    					}
    			...
    			.sorted((a, b)-> {
    				...
    				else if (a.getOccupation()<b.getOccupation())
    					return 1;
    				else
    					return -1;
    			})
    ...
    public void bookTable(int table, int partyId, String time) {
    	this.t.get(table).addBooking(time, partyId);;
    }
    
    public int countServedCustomers() {
    	int n = this.c.values().stream()
    			.filter(x->x.isEated())
    			.mapToInt(Clients::getCount).sum();
//    			.collect(Collectors.toList());
...
//    	}
    ...
    public int estimationError(int table) {
    	int sum=0;
    	int size = this.t.get(table).getTMed().size();
    	for (int i : this.t.get(table).getTMed())
    		sum = sum + i;
        return sum/size;
    }
}
