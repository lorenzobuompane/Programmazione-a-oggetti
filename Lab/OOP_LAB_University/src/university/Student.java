...
public class Student {
	...
	private int nCourse = 0;				//numero corsi registrati
	
	private Course[] studCourse = new Course[MAX_INSxSTUD];		//corsi registrati
	...
	public Boolean ctrlIns (int studentID, int courseCode) {
		...
		for (i=0; i<MAX_INSxSTUD; i++) {
			if (nCourse<=MAX_INSxSTUD) {								//controllo di aver ancora posti per inserire un corso
				...
				if (studCourse[i].ctrlStud(studentID)==true) {			//chiamo funzione per controllo disponibilità corso
		...
		if (nCourse==MAX_INSxSTUD) 
			...
			System.out.println("Corso già registrato!");
		return false;													
	}
