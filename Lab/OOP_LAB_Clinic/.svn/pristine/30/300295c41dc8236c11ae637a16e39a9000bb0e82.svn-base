...
public class Clinic {
	...
	public Collection<Integer> busyDoctors(){	
		double avg = d.values().stream()
				.mapToInt(x->x.getPat().size()).average().getAsDouble();
		return d.values().stream()
				.sorted((a, b)->(a.getLastFirst().compareTo(b.getLastFirst())))
				.filter(x->x.getPat().size()>avg)
				.collect(mapping(x->x.getId(), toList()));
	}
	...
	public Collection<String> countPatientsPerSpecialization(){
		return d.values().stream()
				...
				.map( entry -> String.format("%3d", entry.getValue()) + " - " + entry.getKey())
